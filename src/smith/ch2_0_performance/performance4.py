...
from smith.ch2_0_performance.ch2_0_performance import generate, set_repro, reproducible_step, profile_train, profile_config1, profile_config2, profile_config3, train


class Perf4Transformer(nn.Module):
...
if __name__ == "__main__":
    model_type = Perf4Transformer
    
    # profile_train(model_type, profile_config3, str(type(model_type)))
    # train(model_type, profile_config3, str(type(model_type)))
    # do przegenerowania: /tmp/model_w6j4s6nz.pth
    
    max_seq_len = 256
    ds = load_dataset("Trelis/tiny-shakespeare")
    
    chunks = []
    unique_chars = set(["_eos_", "_bos_"])
    ...
    profile_config3.vocab_size = len(token_to_vocab)
    batch_size = 8

    generated, gen_proba = generate(model_type, 
            profile_config3, 
            checkpoint=f".models/model_w6j4s6nz.pth", 
            device="cpu",
            batch_size=batch_size, 
            max_seq_len=max_seq_len, 
            start_tokens=[vocab_to_token["_bos_"]]
            )
    for g, proba in zip(generated, gen_proba):
        g = g.cpu().detach().numpy().tolist()
        s = "".join([token_to_vocab[g_el] for g_el in g])
        print(s)
        print()
